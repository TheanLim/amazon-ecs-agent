...
func (c *common) buildAWSVPCNetworkNamespaces(
	...
	ifNameMap := make(map[string]acstype.ElasticNetworkInterface, len(taskPayload.ElasticNetworkInterfaces))
	...
	for _, container := range taskPayload.Containers {
		...
		var ifaces []acstype.ElasticNetworkInterface
		for _, ifName := range container.NetworkInterfaceNames {
			if iface, exists := ifNameMap[ifName]; exists {
...
func (c *common) buildNetNS(
	taskID string,
	index int,
	networkInterfaces []acstype.ElasticNetworkInterface,
	proxyConfig *acstype.ProxyConfiguration,
